...
//mot fix
/*
$message = C::t('fb_opinion')->getMessage();

showtableheader('客户留言(只显示最新的30条) <a href="admin.php?action=plugins&operation=config&do=156&identifier=fb_opinion&pmod=manage">处理</a>', 'nobottom fixpadding');
echo '<tr><td>UID</td><td>操作</td><td>日期</td><td>名字</td><td>消息</td></tr>';
foreach ($message as $msg) {
	echo '<tr><td><a href="home.php?mod=space&uid='.$msg['uid'].'&do=profile">'.$msg['uid'].'</a></td><td><a href="plugin.php?id=fb_opinion:main&mod=reply&fbid='.$msg['id'].'" target="blank">答复</a></td><td>'.date('m月d日 h:i' , $msg['dateline']).'</td><td>'.$msg['name'].'</td><td>'.$msg['message'].'</td></tr>';
}
showtablefooter();
*/
