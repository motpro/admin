...
if($_GET['getuid']){
	...
	$give_user=DB::fetch_first('SELECT uid FROM '.DB::table('common_member')." WHERE username='{$_GET[username]}'");
	if (!$give_user['uid']){
	...
	if ($my_credit < ($vip->vars['vip_cost']*$_GET['month'])) showmessage('dsu_kkvip:buy_nomoney','vip.php?action=give_vip');
	$give_user=DB::fetch_first('SELECT uid, groupid FROM '.DB::table('common_member')." WHERE username='{$_GET[username]}'");
	if(!$give_user['uid']) showmessage('dsu_kkvip:uid_not_exists_return');
	$vip->pay_vip($give_user['uid'], $_GET['month']*30, $give_user['groupid']);
	notification_add($give_user['uid'], 'dsu_kkvip', lang('plugin/dsu_kkvip','notification_give'), array('month'=>$_GET['month'], 'username'=>$_G['username']), 1);
	updatemembercount($_G['uid'], array($vip->vars['creditid'] => -$vip->vars['vip_cost']*$_GET['month']), false);
	$trade_succeed	= true;
	$trade_user		= $give_user['uid'];
	showmessage('dsu_kkvip:give_succeed', 'vip.php?action=give_vip', array('month'=>$_GET['month'], 'giveto'=>$_G['gp_username']));
}else{
