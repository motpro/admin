...
class plugin_dsu_kkvip{
	...
	function viewthread_sidetop_output(){
		...
		$_G['setting']['verify']['vip']=array(
			'available'=>true,
			'title'=>'VIP &#20250;&#21592;',
			'icon'=>'source/plugin/dsu_kkvip/images/vip.png',
		);
	...
	function forumdisplay_kkvip_output(){
		...
		foreach ($_G['forum_threadlist'] as $key=>$thread){
			if ($vip->is_vip($thread['authorid'])){
				...
				if($vip->vars['viewthread_redname']) $_G['forum_threadlist'][$key]['author']='<font color="red">'.$thread['author'].'</font>';
				$verify[$thread['authorid']]='&nbsp;<a href="vip.php" target="_blank"><img src="source/plugin/dsu_kkvip/images/vip.png"></a>';
			}
		}
	}
}
