<?php
	...
	//检查人人数据表是否存在
	$rr_tb = DB::table("renren_connect");
	$sql = "show tables like '$rr_tb'";
	...
	if(!defined('RENREN')){
		showMsg($Dlang['renren_no_exists']);
	}
	//////////////检查是否成功安装灯鹭连接 
	...
	//////////////检查人人表字段是否正常
	$sql = "desc ".DB::table("renren_connect");
	$q = DB::query($sql);
	while ($arr=DB::fetch($q)){
		$field[] = $arr['Field'];
	}
	if(!(in_array('rr_uid',$field) && in_array('dz_uid',$field))){
		showMsg($Dlang['renren_table_false']);
	}
	///////////////开始转换
	$sql = "select rr_uid,dz_uid as uid  from $rr_tb  where tag=1 limit $step,$offset";
	$q = DB::query($sql);
	while($a = DB::fetch($q)){
		$ret[$a['uid']] = $a['rr_uid'];/////////////////传参
		$chk[$a['rr_uid']] = $a['uid'];/////////////////////核对
	}
	empty($ret) && showMsg($Dlang['renren_switch_success']);
	
	$x = implode(',',$ret);	
	$userinfo = dfopen("http://open.denglu.cc/api/v2/apiGetMediaData.jsp",15000,"x=$x&m=7",'',true);
	...
	foreach($userinfo as $v){
		...
		$sql = "select uid from ".DB::table("denglu_bind_info")." where uid='$uid' and mediaID=7";
		...
		$r = DB::insert('denglu_bind_info',array('uid'=>$uid,'mediaUserID'=>$v['mediaUserID'],'mediaID'=>7,'screenName'=>$v['screenName'],'tag'=>1,'createtime'=>$dz['regdate'],'is_first'=>$is_first));
		dobind($v,$dz);
	...
	function dobind($v,$dz){
		global $_G;
		loadcache('plugin');
		$url = 'http://open.denglu.cc/api/v2/apiBind.jsp?appid='.$_G['cache']['plugin']['denglu']['appid'].'&muid='.$v['mediaUserID'].'&uname='.$dz['username'].'&email='.$dz['email'];
